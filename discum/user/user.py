...
class User(object):
    ...
    def allowDMsFromServerMembers(self, allow, disallowedGuildIDs):
        url = self.discord + "users/@me/settings"
        body = {
            "restricted_guilds": disallowedGuildIDs,
            "default_guilds_restricted": not allow,
        }
    ...
    def getStripeClientSecret(
        self,
    ):  # for adding new payment methods. Stripe api wraps are not included because discum is just a discord api
    ...
    @staticmethod
    def index(
        inputList, searchItem
    ):  # only used for notification settings, returning -1 doesn't make sense in this context
        try:
