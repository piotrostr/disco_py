class Event:
    ...
    @property
    def channel_read_state_updated(
        self,
    ):  # {'t': 'CHANNEL_UNREAD_UPDATE', 's': s, 'op': 0, 'd': {'guild_id': '', 'channel_unread_updates': [{'last_message_id': '', 'id': 'unread id i guess'}, ...]}}
    ...
    def ban_added(
        self,
    ):  # {'t': 'GUILD_BAN_ADD', 's': s, 'op': 0, 'd': {'user': {'username': username, 'public_flags': 0, 'id': id, 'discriminator': '0000', 'avatar': None}, 'guild_id': guildID}}
    ...
    def guild_member_updated(
        self,
    ):  # {'t': 'GUILD_MEMBER_UPDATE', 's': s, 'op': 0, 'd': {'user': {'username': usernamd, 'public_flags': 0, 'id': id, 'discriminator': '0000', 'avatar': avatar}, 'roles': [], 'premium_since': None, 'pending': False, 'nick': None, 'joined_at': '', 'is_pending': False, 'guild_id': guildID}}
        return self.response["t"] == "GUILD_MEMBER_UPDATE"

    @property
    def guild_members_chunk(
        self,
    ):  # used for op 8 (when searching in the search bar and members come up)
        return self.response["t"] == "GUILD_MEMBERS_CHUNK"

    @property
    def guild_member_removed(
        self,
    ):  # only seems to get sent when you leave a guild (voluntarily or involuntarily)...but the server owner sees no such event? weird.
    ...
    def message_ack(
        self,
    ):  # {'t': 'MESSAGE_ACK', 's': s, 'op': 0, 'd': {'version': v, 'message_id': '', 'channel_id': ''}}
    ...
    @property
    def bulk_messages_deleted(
        self,
    ):  # {'t': 'MESSAGE_DELETE_BULK', 's': s, 'op': 0, 'd': {'ids': [], 'channel_id': '', 'guild_id': ''}}
    ...
    @property
    def message_reaction_emoji_removed(
        self,
    ):  # not entirely sure what's the difference between this and MESSAGE_REACTION_REMOVE but ok
    ...
    @property
    def user_guild_settings_updated(
        self,
    ):  # like notifications for guilds you're in and that sort of stuff
