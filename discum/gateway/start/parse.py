...
class StartParse(object):  # really hope this doesn't take too long to run...
    ...
    def ready(response):
        ...
        user_pool = {
            h["id"]: h for h in response["d"]["users"]
        }  # convert to dict for faster retrieval
        # parse relationships
        ready_data["relationships"] = {
            i["id"]: dict(
                dict(i, **{"type": Types.relationshipTypes[i["type"]]}),
                **user_pool.get(i["id"], {})
            )
    ...
    def ready_supplemental(response):
        ...
        ready_supp_data["online_friends"] = {
            o["user_id"]: o for o in response["d"]["merged_presences"]["friends"]
        }
        ready_supp_data.pop("guilds")
        ready_supp_data["voice_states"] = {
            p["id"]: p.get("voice_states", []) for p in response["d"]["guilds"]
        }  # id is the guild_id
