...
class GuildParse(object):
    ...
    def guild_create(response, my_user_id):
        ...
        guilddata["channels"] = {
            k["id"]: dict(k, **{"type": Types.channelTypes[k["type"]]})
            for k in response["d"]["channels"]
        }
        return guilddata
    ...
    def guild_members_chunk(response):
        memberChunkData = {
            "members": [],
            "guild_id": response["d"]["guild_id"],
            "chunk_count": response["d"]["chunk_count"],
            "chunk_index": response["d"]["chunk_index"],
        }
        if "not_found" in response["d"]:
            memberChunkData["not_found"] = [
                str(n) for n in response["d"]["not_found"]
            ]  # list of user ids
        presences = {}
