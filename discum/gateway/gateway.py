...
class ConnectionResumableException(Exception
                                   ):  # for certain close codes. "exception"
    pass
...
class GatewayServer:
    ...
    __slots__ = [
        ...
        "session",
        "ws",
        "_after_message_hooks",
        "_last_err",
        "_last_close_event",
        "connected",
        "resumable",
        "voice_data",
        "memberFetchingStatus",
        "resetMembersOnSessionReconnect",
        "updateSessionData",
        "guildMemberSearches",
        "_last_ack",
        "latency",
    ...
    class OPCODE:
        ...
        VOICE_STATE_UPDATE = (
            4  #  Send            used to join/move/leave voice channels
        )
        ...
        INVALID_SESSION = (
            9  #  Receive         used to notify client they have an invalid session id
        )
    ...
    def __init__(
        ...
        self.proxy_type, self.proxy_auth, self.proxy_host, self.proxy_port = [
            None
        ] * 4
        if sessionobj and sessionobj.proxies:
            self.proxy_type = proxy_type = list(sessionobj.proxies.keys())[0]
            self.proxy_host, self.proxy_port = (
                sessionobj.proxies[proxy_type].split("://")[-1].split(":"))
            if sessionobj.auth:
                self.proxy_auth = (sessionobj.auth.username,
                                   sessionobj.auth.password)
        ...
        self.voice_data = (
            {})  # voice connections dependent on current (connected) session
        ...
        if not self.resumable:
            ...
            if len(self.session.settings_ready) != 0:
                if self.session.userSettings.get("activities") not in (None,
                                                                       {}):
                    self.auth["presence"][
                        "status"] = self.session.userSettings.get("status")
                    self.auth["presence"]["activities"] = imports.UserCombo(
                        self).constructActivitiesList()
            self.send({"op": self.OPCODE.IDENTIFY, "d": self.auth})
            self.send({
                "op": self.OPCODE.VOICE_STATE_UPDATE,
                "d": {
                    "guild_id": None,
                    "channel_id": None,
                    "self_mute": True,
            ...
            self.send({
                ...
                "d": {
                    ...
                    self.session_id,
                    "seq":
                    self.sequence - 1 if self.sequence > 0 else self.sequence,
                },
            })
    ...
    def on_message(self, ws, message):
        ...
        Logger.log("[gateway] < {}".format(response), LogLevel.RECEIVE,
                   self.log)
        if (response["op"] == self.OPCODE.HELLO
            ):  # only happens once, first message sent to client
            self.interval = (
                response["d"]["heartbeat_interval"]
            ) / 1000  # if this fails make an issue and I'll revert it back to the old method (slightly smaller wait time than heartbeat)
            ...
            Logger.log("[gateway] Received opcode 7 (reconnect).", None,
                       self.log)
            self._last_err = NeedToReconnectException(
                "Discord sent an opcode 7 (reconnect).")
            self.close()
        ...
        if resp.event.ready:
            ...
            if not self.resetMembersOnSessionReconnect and self.session.read(
                ...
                for guildID in settings_ready["guilds"]:
                    settings_ready["guilds"][guildID][
                        "members"] = self.session.guild(guildID).members
    ...
    def on_close(self, ws, close_code, close_msg):
        ...
        if close_code or close_msg:
            ...
            Logger.log("[gateway] close status code: " + str(close_code), None,
                       self.log)
            Logger.log("[gateway] close message: " + str(close_msg), None,
                       self.log)
            if not (4000 < close_code <= 4010):
                self.resumable = True
                self._last_err = ConnectionResumableException(
                    "Connection is resumable.")
            if close_code in (None, 1000, 1001, 1006):
                self._last_err = ConnectionManuallyClosedException(
                    "Disconnection initiated by client using close function.")
    ...
    def _heartbeat(self):
        ...
        while self.connected:
            if (self.interval == None
                ):  # can't replicate the issue so consider this a temp patch
                self.interval = 41.25
    ...
    def close(self):
        self.connected = False
        self.READY = False  # reset self.READY
        if not exceptionChecker(
                self._last_err,
            [InvalidSessionException, NeedToReconnectException]):
            self._last_err = ConnectionManuallyClosedException(
                "Disconnection initiated by client using close function.")
        Logger.log("[gateway] websocket closed", None,
                   self.log)  # don't worry if this message prints twice
        self.ws.close()
    ...
    def command(self, func):
        if callable(func):
            ...
            return func
                ...
                func, dict
        ):  # because I can't figure out out to neatly pass params to decorators :(. Normal behavior still works; use as usual.
    ...
    def _response_loop(self,
                       resp):  # thx ToasterUwU for bringing up dummy threads
        ...
        try:
            if exactMatch:
                ...
                commandsCopy = [
                    ...
                    for i in self._after_message_hooks
                ]  # list of just functions
    ...
    def resetSession(
        self,
    ):  # just resets some variables that in-turn, resets the session (client side). Do not run this while running run().
        self.interval = None
        self.session_id = None
        self.sequence = 0
        self.READY = False  # becomes True once READY_SUPPLEMENTAL is received
        ...
        self.voice_data = {}
        self.resumable = (
            False  # you can't resume anyways without session_id and sequence
        )
        self._last_ack = None
        self.memberFetchingStatus = {"first": []}
        ...
        if auto_reconnect:
            while True:
                try:
                    ...
                    self._last_err = KeyboardInterrupt(
                        "Keyboard Interrupt Error")
                    Logger.log(
                        "[gateway] Connection forcibly closed using Keyboard Interrupt.",
                        None,
                        self.log,
                    )
                    break
                    ...
                    if auto_reconnect:
                        if not exceptionChecker(e, [KeyboardInterrupt]):
                            if exceptionChecker(
                                    e, [ConnectionResumableException]):
                                self._last_err = None
                                waitTime = random.randrange(1, 6)
                                Logger.log(
                                    "[gateway] Connection Dropped. Attempting to resume last valid session in {} seconds."
                                    .format(waitTime),
                                    None,
                                    self.log,
                                )
                                time.sleep(waitTime)
                            elif exceptionChecker(
                                    e, [ConnectionManuallyClosedException]):
                                Logger.log(
                                    "[gateway] Connection forcibly closed using close function.",
                                    None,
                                    self.log,
                                )
                                break
    ...
    def sessionUpdates(self, resp):
        ...
        # guild created
        if resp.event.guild:
            guildData = resp.parsed.guild_create(
                my_user_id=self.session.user["id"]
            )  # user id needed for updating personal roles in that guild
            guildID = guildData["id"]
            voiceStateData = guildData.pop("voice_states", [])
            if (not self.resetMembersOnSessionReconnect
                    and guildID in self.session.guildIDs):
        ...
        # guild deleted
            ...
            if "guilds" in self.keepData:
                self.session.guild(resp.raw["d"]["id"]).updateData(
                    {"removed": True})  # add the indicator
            else:
        ...
        # channel created (either dm or guild channel)
            ...
            if channelData["type"] in ("dm", "group_dm"):  # dm
                ...
                self.session.guild(guildID).setChannelData(
                    channelID, channelData)
        # channel deleted (either dm or guild channel)
        elif resp.event.channel_deleted:
            channelData = (resp.parsed.channel_delete()
                           )  # updated data :) ...unlike guild_delete events
            ...
            if channelData["type"] in ("dm", "group_dm"):  # dm
                ...
                if "guild_channels" in self.keepData:
                    self.session.guild(guildID).updateChannelData(
                        channelID, channelData)
                else:
        ...
        # user session replaced (useful for syncing activities btwn client and server)
            ...
            newStatus = resp.parsed.sessions_replace(
                session_id=self.session_id
            )  # contains both status and activities
    ...
    # op14 related stuff
    def getMemberFetchingParams(
        self, targetRangeStarts
    ):  # more for just proof of concept. targetRangeStarts must not contain duplicates and must be a list of integers
        targetRangeStarts = {i: 1
                             for i in targetRangeStarts
                             }  # remove duplicates but preserve order
        if targetRangeStarts.get(0) != None and targetRangeStarts.get(
                100) != None:
            keys = list(targetRangeStarts)
            if keys.index(100) < keys.index(0):
                targetRangeStarts.pop(
                    0
                )  # needs to be removed or else fetchMembers will enter an infinite loop because of how discord responds to member list requests
    ...
    def fetchMembers(
        ...
        method="overlap",
        keep=[],
        considerUpdates=True,
        startIndex=0,
        stopIndex=1000000000,
        reset=True,
        wait=None,
        priority=0,
    ):
        if guild_id in self.memberFetchingStatus and reset:
            del self.memberFetchingStatus[
                guild_id]  # just resetting tracker on the specific guild_id
        self.command({
    ...
    def findVisibleChannels(
        ...
        if len(self.session.read()[0]) == 0:  # if never connected to gateway
            return
        return imports.GuildCombo(self).findVisibleChannels(
            guildID, types, findFirst)
    ...
    # op8 related stuff
    def queryGuildMembers(
        self,
        guildIDs,
        query,
        saveAsQueryOverride=None,
        limit=10,
        presences=True,
        keep=[],
    ):
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        imports.GuildCombo(self).searchGuildMembers(guildIDs, query,
                                                    saveAsQueryOverride, limit,
                                                    presences, None, keep)

    def checkGuildMembers(self, guildIDs, userIDs, presences=True, keep=[]):
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        imports.GuildCombo(self).searchGuildMembers(guildIDs, "", None, 10,
                                                    presences, userIDs, keep)

    def finishedGuildSearch(self,
                            guildIDs,
                            query="",
                            saveAsQueryOverride=None,
                            userIDs=None,
        ...
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        saveAsQuery = (query.lower() if saveAsQueryOverride == None else
        ...
        if (
                keep == False
        ):  # if keep param not provided, look if params are subset of command_list function params
            command["params"].pop("keep")
            for c in self._after_message_hooks:
                if isinstance(c, dict):
                    if (c.get("function").__func__ == imports.GuildCombo(
                            self).handleGuildMemberSearches.__func__):
                        d1 = command["params"]
                        d2 = c.get("params", {})
                        if all(
                                key in d2 and d2[key] == d1[key] for key in d1
                        ):  # https://stackoverflow.com/a/41579450/14776493
    ...
    def setStatus(self, status):  # can only be run while connected to gateway
        imports.User(self.RESTurl, self.sessionobj,
                     self.log).setStatusHelper(status)
        imports.UserCombo(self).setStatus(status)

    def setPlayingStatus(
        self, game
    ):  # can only be run while connected to gateway, will update metadata later
        if not self.session.userSettings["show_current_game"]:
            imports.User(self.RESTurl, self.sessionobj,
                         self.log).enableActivityDisplay(enable=True)
        imports.UserCombo(self).setPlayingStatus(game)

    def removePlayingStatus(
            self):  # can only be run while connected to gateway
        imports.UserCombo(self).removePlayingStatus()

    def setStreamingStatus(
        self, stream, url
    ):  # can only be run while connected to gateway, will update metadata later
        if not self.session.userSettings["show_current_game"]:
            imports.User(self.RESTurl, self.sessionobj,
    ...
    def removeStreamingStatus(
            self):  # can only be run while connected to gateway
        imports.UserCombo(self).removeStreamingStatus()

    def setListeningStatus(
        self, song
    ):  # can only be run while connected to gateway, will update metadata later
        if not self.session.userSettings["show_current_game"]:
            imports.User(self.RESTurl, self.sessionobj,
    ...
    def removeListeningStatus(
            self):  # can only be run while connected to gateway
        imports.UserCombo(self).removeListeningStatus()

    def setWatchingStatus(
        self, show
    ):  # can only be run while connected to gateway, will update metadata later
        if not self.session.userSettings["show_current_game"]:
            imports.User(self.RESTurl, self.sessionobj,
    ...
    def setCustomStatus(
            ...
            emoji=None,
            animatedEmoji=False,
            expires_at=None):  # can only be run while connected to gateway
        imports.User(self.RESTurl, self.sessionobj, self.log).setStatusHelper(
            self.session.userSettings["status"])
        imports.User(self.RESTurl, self.sessionobj,
                     self.log).setCustomStatusHelper(customstatus, emoji,
                                                     expires_at)
        imports.UserCombo(self).setCustomStatus(customstatus, emoji,
                                                animatedEmoji)

    def removeCustomStatus(self):
        imports.User(self.RESTurl, self.sessionobj,
                     self.log).setCustomStatusHelper("")
