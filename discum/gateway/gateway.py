...
class GatewayServer:
        ...
        if not self.resumable:
            ...
            if len(self.session.settings_ready) != 0:
                if self.session.userSettings.get("activities") not in (None, {}):
                    self.auth["presence"]["status"] = self.session.userSettings.get(
                        "status"
                    )
                    self.auth["presence"]["activities"] = imports.UserCombo(
                        self
                    ).constructActivitiesList()
        ...
        if resp.event.ready:
            ...
            if not self.resetMembersOnSessionReconnect and self.session.read()[0]:
                for guildID in settings_ready["guilds"]:
                    settings_ready["guilds"][guildID]["members"] = self.session.guild(
                        guildID
                    ).members
    ...
    def on_close(self, ws, close_code, close_msg):
        ...
        if close_code or close_msg:
            ...
            Logger.log(
                "[gateway] close status code: " + str(close_code), None, self.log
            )
    ...
    def close(self):
        ...
        if not exceptionChecker(
            self._last_err, [InvalidSessionException, NeedToReconnectException]
        ):
            self._last_err = ConnectionManuallyClosedException(
                "Disconnection initiated by client using close function."
            )
        Logger.log(
            "[gateway] websocket closed", None, self.log
        )  # don't worry if this message prints twice
        self.ws.close()
                    ...
                    if auto_reconnect:
                        if not exceptionChecker(e, [KeyboardInterrupt]):
                            if exceptionChecker(e, [ConnectionResumableException]):
                                ...
                                Logger.log(
                                    "[gateway] Connection Dropped. Attempting to resume last valid session in {} seconds.".format(
                                        waitTime
                                    ),
                                    None,
                                    self.log,
    ...
    def sessionUpdates(self, resp):
        ...
        if resp.event.guild:
            ...
            if (
                not self.resetMembersOnSessionReconnect
    ...
    def getMemberFetchingParams(
        ...
        targetRangeStarts = {
            i: 1 for i in targetRangeStarts
    ...
    def fetchMembers(
        ...
        if guild_id in self.memberFetchingStatus and reset:
            del self.memberFetchingStatus[
                guild_id
            ]  # just resetting tracker on the specific guild_id
        self.command(
            {
    ...
    def queryGuildMembers(
        ...
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        imports.GuildCombo(self).searchGuildMembers(
            guildIDs, query, saveAsQueryOverride, limit, presences, None, keep
        )
    ...
    def checkGuildMembers(self, guildIDs, userIDs, presences=True, keep=[]):
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        imports.GuildCombo(self).searchGuildMembers(
            guildIDs, "", None, 10, presences, userIDs, keep
        )

    def finishedGuildSearch(
        self, guildIDs, query="", saveAsQueryOverride=None, userIDs=None, keep=False
    ):
        if isinstance(guildIDs, str):
            guildIDs = [guildIDs]
        saveAsQuery = (
            query.lower()
            if saveAsQueryOverride == None
            else saveAsQueryOverride.lower()
        )
        command = {
        ...
        if (
            ...
            for c in self._after_message_hooks:
                if isinstance(c, dict):
                    if (
                        c.get("function").__func__
                        == imports.GuildCombo(self).handleGuildMemberSearches.__func__
                    ):
                        d1 = command["params"]
                        d2 = c.get("params", {})
                        if all(
    ...
    def setPlayingStatus(
        ...
        if not self.session.userSettings["show_current_game"]:
            imports.User(self.RESTurl, self.sessionobj, self.log).enableActivityDisplay(
                enable=True
            )
    ...
    def setCustomStatus(
        self, customstatus, emoji=None, animatedEmoji=False, expires_at=None
    ):  # can only be run while connected to gateway
