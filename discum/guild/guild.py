...
class Guild(object):
    ...
    def joinGuild(self, inviteCode, location, wait):
        ...
        if location in ("accept invite page", "join guild"):
            guildData = self.getInfoFromInviteCode(
                inviteCode,
                with_counts=True,
                with_expiration=True,
                fromJoinGuildNav=(location.lower() == "join guild"),
            ).json()
            if wait:
                time.sleep(wait)
            return self.joinGuildRaw(
                inviteCode,
                guildData["guild"]["id"],
                guildData["channel"]["id"],
                guildData["channel"]["type"],
                location,
            )
        elif location == "markdown":
    ...
    def createInvite(
        ...
        checkInvite,
        targetType,
    ):  # has to be a channel thats in a guild. also checkInvite and targetType are basically useless.
    ...
    def kick(self, guildID, userID, reason):
        url = self.discord + "guilds/%s/members/%s?reason=%s" % (
            guildID,
            userID,
            quote(reason),
        )
    ...
    def createThread(self, channelID, name, messageID, public, archiveAfter):
        ...
        body = {
            "name": name,
            "type": threadType,
            "auto_archive_duration": archiveAfterSeconds,
        }
        return Wrapper.sendRequest(self.s, "post", url, body, log=self.log)
    ...
    def getSchoolHubDirectoryCounts(
        self, hubID
    ):  # this only returns the # of guilds/groups in each directory/category. This doesn't even return the category names
