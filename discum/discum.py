...
class Client:
    ...
    def __init__(
        ...
        self.websocketurl = "wss://gateway.discord.gg/?encoding=json&v={}&compress=zlib-stream".format(
            self.api_version
        )
        ...
        # step 4: proxies
        self.switchProxy(
            proxy, updateGateway=False
        )  # gateway proxies updated on step 8
        ...
        self.__super_properties = self.getSuperProperties(
            self.__user_agent, build_num, self.locale
        )
        self.s.headers.update(
            {
                "X-Super-Properties": base64.b64encode(
                    json.dumps(self.__super_properties).encode()
                ).decode("utf-8")
            }
        )

        # step 7: token/authorization/fingerprint (also part of headers, except for fingerprint)
        login_needed = token in ("", None, False) and {email, password}.isdisjoint(
            {"", None, False}
        )
        if login_needed:
            if remote_auth:
                ...
                loginResponse, self.__xfingerprint = self.login(
                    email, password, False, None, None, None, secret, code
                )
                self.s.cookies.update(loginResponse.cookies)
                self.__user_token = loginResponse.json().get(
                    "token"
                )  # update token from "" to actual value
    ...
    def checkToken(self, token):
        editedS = imports.Wrapper().editedReqSession(
            self.s, {"update": {"Authorization": token}}
        )
        other = imports.Other(editedS, self.discord, self.log)
        test = (
            other.getLibrary()
        )  # https://github.com/vida1338/discord-token-checker/blob/main/checker.py#L46
    ...
    def switchProxy(self, newProxy, updateGateway=True):
        ...
        if search:
            ...
            # proxy updating
            proxies = {
                t: "{}://{}:{}".format(t, proxy_host, proxy_port) for t in proxy_type
            }
    ...
    def login(
        ...
        return imports.Login(self.s, self.discord, self.log).login(
            email, password, undelete, captcha, source, gift_code_sku_id, secret, code
        )
    ...
    def getSuperProperties(self, user_agent, buildnum="request", locale=None):
        return imports.SuperProperties(self.s, buildnum, self.log).getSuperProperties(
            user_agent, locale
        )  # self.locale
    ...
    def getVersionStableHash(self, underscore=None):
        return imports.Other(self.s, self.discord, self.log).getVersionStableHash(
            underscore
        )
    ...
    def removeFromDmGroup(self, channelID, userID):
        return imports.Messages(self.discord, self.s, self.log).removeFromDmGroup(
            channelID, userID
        )

    # add to DM group
    def addToDmGroup(self, channelID, userID):
        return imports.Messages(self.discord, self.s, self.log).addToDmGroup(
            channelID, userID
        )

    # create DM group invite link
    def createDmGroupInvite(self, channelID, max_age_seconds=86400):
        return imports.Messages(self.discord, self.s, self.log).createDmGroupInvite(
            channelID, max_age_seconds
        )

    # change DM group name
    def setDmGroupName(self, channelID, name):
        return imports.Messages(self.discord, self.s, self.log).setDmGroupName(
            channelID, name
        )

    # change DM icon
    def setDmGroupIcon(self, channelID, imagePath):
        return imports.Messages(self.discord, self.s, self.log).setDmGroupIcon(
            channelID, imagePath
        )

    # get recent messages
    def getMessages(
        self, channelID, num=1, beforeDate=None, aroundMessage=None
    ):  # num <= 100, beforeDate is a snowflake
        return imports.Messages(self.discord, self.s, self.log).getMessages(
            channelID, num, beforeDate, aroundMessage
        )

    # get message by channel ID and message ID
    def getMessage(self, channelID, messageID):
        return imports.Messages(self.discord, self.s, self.log).getMessage(
            channelID, messageID
        )

    # greet with stickers
    def greet(self, channelID, sticker_ids=["749054660769218631"]):
        return imports.Messages(self.discord, self.s, self.log).greet(
            channelID, sticker_ids
        )
    ...
    def sendMessage(
        ...
        return imports.Messages(self.discord, self.s, self.log).sendMessage(
            channelID,
            message,
            nonce,
            tts,
    ...
    def sendFile(
        ...
        return imports.Messages(self.discord, self.s, self.log).sendFile(
            channelID, filelocation, isurl, message, tts, message_reference, sticker_ids
        )
    ...
    def reply(
        ...
        return imports.Messages(self.discord, self.s, self.log).reply(
            channelID,
            messageID,
            message,
            nonce,
    ...
    def searchMessages(
        ...
        beforeDate=None,
        textSearch=None,
        afterNumResults=None,
        limit=None,
    ):
        return imports.Messages(self.discord, self.s, self.log).searchMessages(
            guildID,
            channelID,
            authorID,
            authorType,
    ...
    def filterSearchResults(self, searchResponse):
        return imports.Messages(self.discord, self.s, self.log).filterSearchResults(
            searchResponse
        )
    ...
    def deleteMessage(self, channelID, messageID):
        return imports.Messages(self.discord, self.s, self.log).deleteMessage(
            channelID, messageID
        )

    # edit message
    def editMessage(self, channelID, messageID, newMessage="", newEmbed=None):
        return imports.Messages(self.discord, self.s, self.log).editMessage(
            channelID, messageID, newMessage, newEmbed
        )

    # pin message
    def pinMessage(self, channelID, messageID):
        return imports.Messages(self.discord, self.s, self.log).pinMessage(
            channelID, messageID
        )

    # un-pin message
    def unPinMessage(self, channelID, messageID):
        return imports.Messages(self.discord, self.s, self.log).unPinMessage(
            channelID, messageID
        )
    ...
    def addReaction(self, channelID, messageID, emoji):
        return imports.Messages(self.discord, self.s, self.log).addReaction(
            channelID, messageID, emoji
        )

    # remove reaction
    def removeReaction(self, channelID, messageID, emoji):
        return imports.Messages(self.discord, self.s, self.log).removeReaction(
            channelID, messageID, emoji
        )

    # get reaction users
    def getReactionUsers(
        self, channelID, messageID, emoji, afterUserID=None, limit=100
    ):
        return imports.Messages(self.discord, self.s, self.log).getReactionUsers(
            channelID, messageID, emoji, afterUserID, limit
        )

    # acknowledge message (mark message read)
    def ackMessage(self, channelID, messageID, ackToken=None):
        return imports.Messages(self.discord, self.s, self.log).ackMessage(
            channelID, messageID, ackToken
        )

    # unacknowledge message (mark message unread)
    def unAckMessage(self, channelID, messageID, numMentions=0):
        return imports.Messages(self.discord, self.s, self.log).unAckMessage(
            channelID, messageID, numMentions
        )
    ...
    def getTrendingGifs(self, provider="tenor", locale="en-US", media_format="mp4"):
        return imports.Messages(self.discord, self.s, self.log).getTrendingGifs(
    ...
    def getStickers(
        self, directoryID="758482250722574376", store_listings=False, locale="en-US"
    ):
        return imports.Stickers(self.discord, self.s, self.log).getStickers(
            directoryID, store_listings, locale
        )

    def getStickerFile(self, stickerID, stickerAsset):  # this is an animated png
        return imports.Stickers(self.discord, self.s, self.log).getStickerFile(
            stickerID, stickerAsset
        )

    def getStickerJson(self, stickerID, stickerAsset):
        return imports.Stickers(self.discord, self.s, self.log).getStickerJson(
            stickerID, stickerAsset
        )

    def getStickerPack(self, stickerPackID):
        return imports.Stickers(self.discord, self.s, self.log).getStickerPack(
            stickerPackID
        )
    ...
    def getProfile(self, userID, with_mutual_guilds=True, guildID=None):
        return imports.User(self.discord, self.s, self.log).getProfile(
            userID, with_mutual_guilds, guildID
        )
    ...
    def getMentions(self, limit=25, roleMentions=True, everyoneMentions=True):
        return imports.User(self.discord, self.s, self.log).getMentions(
            limit, roleMentions, everyoneMentions
        )

    def removeMentionFromInbox(self, messageID):
        return imports.User(self.discord, self.s, self.log).removeMentionFromInbox(
            messageID
        )
    ...
    def setUsername(self, username):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s, self.log).setUsername(
            username, password=self.__user_password
        )

    # set email
    def setEmail(self, email):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s, self.log).setEmail(
            email, password=self.__user_password
        )

    # set password
    def setPassword(
        self, new_password
    ):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s, self.log).setPassword(
            new_password, password=self.__user_password
        )

    # set discriminator
    def setDiscriminator(
        self, discriminator
    ):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s, self.log).setDiscriminator(
            discriminator, password=self.__user_password
        )
    ...
    # 2FA
    def calculateTOTPcode(
        self, secret="default"
    ):  # need to put this function here (instead of in login folder or user folder) because it updates the secret (if and only if secret == "")
        if secret == "default":
            if self.__totp_secret == "":
                self.__totp_secret = "".join(
                    random.choice(list("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567"))
                    for _ in range(16)
                )  # random base32 (len 16)
            secret = self.__totp_secret
        if "?secret=" in secret:
            secret = secret[
                secret.index("?secret=") + 8 : secret.index("?secret=") + 24
            ]
        return (
            imports.TOTP(secret).generateTOTP(),
            secret,
        )  # secret is returned just in case it wasn't set at the beginning.

    def getTOTPurl(
        self, secret
    ):  # use this to store your totp secret/qr pic; btw url format is otpauth://totp/Discord:EMAIL?secret=SECRET&issuer=Discord
    ...
    def enable2FA(
        self,
    ):  # this also returns backup codes (value of key "backup_codes"). USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        code = self.calculateTOTPcode()[0]
        result = imports.User(self.discord, self.s, self.log).enable2FA(
            code, secret=self.__totp_secret, password=self.__user_password
        )
    ...
    def disable2FA(
        self, code="calculate", clearSecretAfter=False
    ):  # either set your token before running this or input a code.
        if code == "calculate":
            code = self.calculateTOTPcode()[
                0
            ]  # this will generate a random secret if you dont have one set, so...set your secret before running this
        ...
        if (
            clearSecretAfter
        ):  # this is dangerous (even though disable2FA should error out before getting to this point if something goes wrong). If you already have your secret saved, clear away. By default this is set to False to avoid any mishaps.
            self.__totp_secret = ""
        return result

    def getBackupCodes(self, regenerate=False):
        return imports.User(self.discord, self.s, self.log).getBackupCodes(
            self.__user_password, regenerate
        )
    ...
    def validatePhone(self, number, code, password):
        result = imports.User(self.discord, self.s, self.log).validatePhone(
    ...
    def allowDMsFromServerMembers(self, allow=True, disallowedGuildIDs=None):
        return imports.User(self.discord, self.s, self.log).allowDMsFromServerMembers(
            allow, disallowedGuildIDs
        )

    def allowFriendRequestsFrom(
        self, types=["everyone", "mutual_friends", "mutual_guilds"]
    ):
        return imports.User(self.discord, self.s, self.log).allowFriendRequestsFrom(
            types
        )

    def analyticsConsent(self, grant=[], revoke=[]):
        return imports.User(self.discord, self.s, self.log).analyticsConsent(
            grant, revoke
        )

    def allowScreenReaderTracking(self, allow=True):
        return imports.User(self.discord, self.s, self.log).allowScreenReaderTracking(
            allow
        )
    ...
    def getConnectionUrl(self, accountType):
        return imports.User(self.discord, self.s, self.log).getConnectionUrl(
            accountType
        )

    def enableConnectionDisplayOnProfile(
        self, accountType, accountUsername, enable=True
    ):
        return imports.User(
            self.discord, self.s, self.log
        ).enableConnectionDisplayOnProfile(accountType, accountUsername, enable)

    def enableConnectionDisplayOnStatus(
        self, accountType, accountUsername, enable=True
    ):
        return imports.User(
            self.discord, self.s, self.log
        ).enableConnectionDisplayOnStatus(accountType, accountUsername, enable)

    def removeConnection(self, accountType, accountUsername):
        return imports.User(self.discord, self.s, self.log).removeConnection(
            accountType, accountUsername
        )
    ...
    def setMessageDisplay(self, CozyOrCompact):  # "cozy" or "compact"
        return imports.User(self.discord, self.s, self.log).setMessageDisplay(
            CozyOrCompact
        )
    ...
    def enableLinkedImageDisplay(self, enable=True):  # boolean, default=True
        return imports.User(self.discord, self.s, self.log).enableLinkedImageDisplay(
            enable
        )
    ...
    def enableReactionRendering(self, enable=True):  # boolean, default=True
        return imports.User(self.discord, self.s, self.log).enableReactionRendering(
            enable
        )

    def enableEmoticonConversion(self, enable=True):  # boolean, default=True
        return imports.User(self.discord, self.s, self.log).enableEmoticonConversion(
            enable
        )

    def setAFKtimeout(self, timeout_seconds):
        return imports.User(self.discord, self.s, self.log).setAFKtimeout(
            timeout_seconds
        )
    ...
    def activateApplicationTestMode(self, applicationID):
        return imports.User(self.discord, self.s, self.log).activateApplicationTestMode(
            applicationID
        )

    def getApplicationData(self, applicationID, with_guild=False):
        return imports.User(self.discord, self.s, self.log).getApplicationData(
            applicationID, with_guild
        )

    def enableActivityDisplay(self, enable=True):
        return imports.User(self.discord, self.s, self.log).enableActivityDisplay(
            enable
        )
    ...
    def suppressEveryonePings(self, guildID, suppress=True):
        return imports.User(self.discord, self.s, self.log).suppressEveryonePings(
            guildID, suppress
        )

    def suppressRoleMentions(self, guildID, suppress=True):
        return imports.User(self.discord, self.s, self.log).suppressRoleMentions(
            guildID, suppress
        )
    ...
    def enableMobilePushNotifications(self, guildID, enable=True):
        return imports.User(
            self.discord, self.s, self.log
        ).enableMobilePushNotifications(guildID, enable)

    def setChannelNotificationOverrides(self, guildID, overrides):
        return imports.User(
            self.discord, self.s, self.log
        ).setChannelNotificationOverrides(guildID, overrides)

    def setMessageNotifications(self, guildID, notifications):
        return imports.User(self.discord, self.s, self.log).setMessageNotifications(
            guildID, notifications
        )

    def muteGuild(self, guildID, mute=True, duration=None):
        return imports.User(self.discord, self.s, self.log).muteGuild(
            guildID, mute, duration
        )
    ...
    def setThreadNotifications(self, threadID, notifications):
        return imports.User(self.discord, self.s, self.log).setThreadNotifications(
    ...
    def reportSpam(
        ...
        return imports.User(self.discord, self.s, self.log).reportSpam(
            channelID, messageID, reportType, guildID, version, variant, language
        )
    ...
    def inviteToCall(self, channelID, userIDs=None):
        return imports.User(self.discord, self.s, self.log).inviteToCall(
            channelID, userIDs
        )
    ...
    def logout(self, provider=None, voip_provider=None):
        return imports.User(self.discord, self.s, self.log).logout(
    ...
    def getInfoFromInviteCode(
        self, inviteCode, with_counts=True, with_expiration=True, fromJoinGuildNav=False
    ):
        return imports.Guild(self.discord, self.s, self.log).getInfoFromInviteCode(
            inviteCode, with_counts, with_expiration, fromJoinGuildNav
        )

    # join guild with invite code
    def joinGuild(self, inviteCode, location="accept invite page", wait=0):
        return imports.Guild(self.discord, self.s, self.log).joinGuild(
            inviteCode, location, wait
        )

    # preview/lurk-join guild. Only applies to current (gateway) session
    def previewGuild(self, guildID, sessionID=None):
        return imports.Guild(self.discord, self.s, self.log).previewGuild(
            guildID, sessionID
        )

    # leave guild
    def leaveGuild(self, guildID, lurking=False):
        return imports.Guild(self.discord, self.s, self.log).leaveGuild(
            guildID, lurking
        )
    ...
    def createInvite(
        ...
        return imports.Guild(self.discord, self.s, self.log).createInvite(
            channelID,
            max_age_seconds,
            max_uses,
            grantTempMembership,
    ...
    def getChannelInvites(self, channelID):
        return imports.Guild(self.discord, self.s, self.log).getChannelInvites(
            channelID
        )
    ...
    def getDiscoverableGuilds(self, offset=0, limit=24):
        return imports.Guild(self.discord, self.s, self.log).getDiscoverableGuilds(
            offset, limit
        )
    ...
    def createGuild(
        ...
        return imports.Guild(self.discord, self.s, self.log).createGuild(
            name, icon, channels, systemChannelID, template
        )
    ...
    def kick(self, guildID, userID, reason=""):
        return imports.Guild(self.discord, self.s, self.log).kick(
            guildID, userID, reason
        )

    # ban a user
    def ban(self, guildID, userID, deleteMessagesDays=0, reason=""):
        return imports.Guild(self.discord, self.s, self.log).ban(
            guildID, userID, deleteMessagesDays, reason
        )
    ...
    def getRoleMemberCounts(self, guildID):
        return imports.Guild(self.discord, self.s, self.log).getRoleMemberCounts(
            guildID
        )

    # get integrations (includes applications aka bots)
    def getGuildIntegrations(self, guildID, include_applications=True):
        return imports.Guild(self.discord, self.s, self.log).getGuildIntegrations(
            guildID, include_applications
        )
    ...
    def getRoleMemberIDs(self, guildID, roleID):
        return imports.Guild(self.discord, self.s, self.log).getRoleMemberIDs(
            guildID, roleID
        )

    # add members to role (add a role to multiple members at the same time)
    def addMembersToRole(self, guildID, roleID, memberIDs):
        return imports.Guild(self.discord, self.s, self.log).addMembersToRole(
            guildID, roleID, memberIDs
        )

    # set roles of a member
    def setMemberRoles(self, guildID, memberID, roleIDs):
        return imports.Guild(self.discord, self.s, self.log).setMemberRoles(
            guildID, memberID, roleIDs
        )

    def getMemberVerificationData(self, guildID, with_guild=False, invite_code=None):
        return imports.Guild(self.discord, self.s, self.log).getMemberVerificationData(
            guildID, with_guild, invite_code
        )

    def agreeGuildRules(
        self, guildID, form_fields, version="2021-01-05T01:44:32.163000+00:00"
    ):
        return imports.Guild(self.discord, self.s, self.log).agreeGuildRules(
            guildID, form_fields, version
        )

    def createThread(
        self, channelID, name, messageID=None, public=True, archiveAfter="24 hours"
    ):
        return imports.Guild(self.discord, self.s, self.log).createThread(
            channelID, name, messageID, public, archiveAfter
        )

    def leaveThread(self, threadID, location="Sidebar Overflow"):
        return imports.Guild(self.discord, self.s, self.log).leaveThread(
            threadID, location
        )

    def joinThread(self, threadID, location="Banner"):
        return imports.Guild(self.discord, self.s, self.log).joinThread(
            threadID, location
        )

    def archiveThread(self, threadID, lock=True):
        return imports.Guild(self.discord, self.s, self.log).archiveThread(
            threadID, lock
        )

    def unarchiveThread(self, threadID, lock=False):
        return imports.Guild(self.discord, self.s, self.log).unarchiveThread(
            threadID, lock
        )

    def lookupSchool(self, email, allowMultipleGuilds=True, useVerificationCode=True):
        return imports.Guild(self.discord, self.s, self.log).lookupSchool(
            email, allowMultipleGuilds, useVerificationCode
        )

    def schoolHubWaitlistSignup(self, email, school):
        return imports.Guild(self.discord, self.s, self.log).schoolHubWaitlistSignup(
            email, school
        )

    def schoolHubSignup(self, email, hubID):
        return imports.Guild(self.discord, self.s, self.log).schoolHubSignup(
            email, hubID
        )

    def verifySchoolHubSignup(self, hubID, email, code):
        return imports.Guild(self.discord, self.s, self.log).verifySchoolHubSignup(
            hubID, email, code
        )
    ...
    def getSchoolHubDirectoryCounts(self, hubID):
        return imports.Guild(
            self.discord, self.s, self.log
        ).getSchoolHubDirectoryCounts(hubID)

    def joinGuildFromSchoolHub(self, hubID, guildID):
        return imports.Guild(self.discord, self.s, self.log).joinGuildFromSchoolHub(
            hubID, guildID
        )

    def searchSchoolHub(self, hubID, query):
        return imports.Guild(self.discord, self.s, self.log).searchSchoolHub(
            hubID, query
        )
    ...
    def setSchoolHubGuildDetails(self, hubID, guildID, description, directoryID):
        return imports.Guild(self.discord, self.s, self.log).setSchoolHubGuildDetails(
            hubID, guildID, description, directoryID
        )
    ...
    def getGuildActivitiesConfig(self, guildID):
        return imports.Guild(self.discord, self.s, self.log).getGuildActivitiesConfig(
            guildID
        )
    ...
    def getSlashCommands(self, applicationID):
        return imports.SlashCommands(self.discord, self.s, self.log).getSlashCommands(
            applicationID
        )
    ...
    def triggerSlashCommand(
        ...
        return imports.SlashCommands(
            self.discord, self.s, self.log
        ).triggerSlashCommand(applicationID, channelID, guildID, data, nonce, sessionID)

    # trigger a user command (right click on bot username, select a command from Apps)
    def triggerUserCommand(
        self,
        applicationID,
        channelID,
        guildID=None,
    ...
    def click(
        ...
        return imports.Buttons(self.discord, self.s, self.log).click(
            applicationID,
            channelID,
            messageID,
            messageFlags,
    ...
    def initScience(self):
        try:
            # get analytics token
            response = imports.User(self.discord, self.s, self.log).info(
            ...
            self.userData = (
                response.json()
            )  # this is essentially the connection test. We need it cause we can get important data without connecting to the gateway.
        except:
            self.userData = {"analytics_token": None, "id": "0"}  # if token invalid
        # initialize Science object
        self.Science = imports.Science(
            self.discord,
            self.s,
            self.log,
            self.userData["analytics_token"],
            self.userData["id"],
        )

    def science(
        self, events=[{}]
    ):  # the real prep for science events happens down here, and only once for each client obj
        if events == [{}]:
            events = [{}]
        if self.Science == "":
            self.initScience()
    ...
    def parseClientUUID(self, client_uuid):
        if self.Science == "":
            self.Science = imports.Science(
                self.discord, self.s, self.log, None, "0", ""
