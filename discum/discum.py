...
class Client:
    ...
    def __init__(
        ...
        headers = {
            ...
            "Sec-Ch-Ua":
            '" Not A;Brand";v="99", "Chromium";v="{0}", "Google Chrome";v="{0}"'
            .format(parsed_ua["user_agent"]["major"]),
            "Sec-Ch-Ua-Mobile":
            "?0",
            "Sec-Ch-Ua-Platform":
            '"{}"'.format(parsed_ua["os"]["family"]),
            "Sec-Fetch-Dest":
            "empty",
            "Sec-Fetch-Mode":
            "cors",
        ...
        self.s.headers.update({
            ...
            base64.b64encode(json.dumps(
                self.__super_properties).encode()).decode("utf-8")
        })

        # step 7: token/authorization/fingerprint (also part of headers, except for fingerprint)
        login_needed = token in ("", None, False) and {
            email, password
        }.isdisjoint({"", None, False})
        if login_needed:
            if remote_auth:
                self.__user_token, self.userData = self.remoteAuthLogin(
                    remote_auth)
            else:
                ...
                self.__user_token = loginResponse.json().get(
                    "token")  # update token from "" to actual value
    ...
    def switchProxy(self, newProxy, updateGateway=True):
        ...
        if search:
            ...
            # proxy updating
            proxies = {
                t: "{}://{}:{}".format(t, proxy_host, proxy_port)
                for t in proxy_type
            }
    ...
    def login(
        ...
        return imports.Login(self.s, self.discord,
                             self.log).login(email, password, undelete,
                                             captcha, source, gift_code_sku_id,
                                             secret, code)

    def getXFingerprint(self, generateIfNone=False):
        return imports.Login(self.s, self.discord,
                             self.log).getXFingerprint(generateIfNone)

    def getBuildNumber(self):
        return imports.SuperProperties(self.s, "request",
                                       self.log).requestBuildNumber()

    def getSuperProperties(self, user_agent, buildnum="request", locale=None):
        return imports.SuperProperties(self.s, buildnum,
                                       self.log).getSuperProperties(
                                           user_agent, locale)  # self.locale
    ...
    def getVersionStableHash(self, underscore=None):
        return imports.Other(self.s, self.discord,
                             self.log).getVersionStableHash(underscore)
    ...
    def getMessages(
            ...
            num=1,
            beforeDate=None,
            aroundMessage=None):  # num <= 100, beforeDate is a snowflake
        return imports.Messages(self.discord, self.s,
                                self.log).getMessages(channelID, num,
                                                      beforeDate,
                                                      aroundMessage)
    ...
    def getMessage(self, channelID, messageID):
        return imports.Messages(self.discord, self.s,
                                self.log).getMessage(channelID, messageID)

    # greet with stickers
    def greet(self, channelID, sticker_ids=["749054660769218631"]):
        return imports.Messages(self.discord, self.s,
                                self.log).greet(channelID, sticker_ids)
    ...
    def sendFile(
        ...
        return imports.Messages(self.discord, self.s,
                                self.log).sendFile(channelID, filelocation,
                                                   isurl, message, tts,
                                                   message_reference,
                                                   sticker_ids)
    ...
    def getStickers(self,
                    ...
                    store_listings=False,
                    locale="en-US"):
        return imports.Stickers(self.discord, self.s,
                                self.log).getStickers(directoryID,
                                                      store_listings, locale)

    def getStickerFile(self, stickerID,
                       stickerAsset):  # this is an animated png
    ...
    def getProfile(self, userID, with_mutual_guilds=True, guildID=None):
        return imports.User(self.discord, self.s,
                            self.log).getProfile(userID, with_mutual_guilds,
                                                 guildID)

    def info(self, with_analytics_token=None):
        return imports.User(self.discord, self.s,
                            self.log).info(with_analytics_token)
    ...
    def setUserNote(self, userID, note):
        return imports.User(self.discord, self.s,
                            self.log).setUserNote(userID, note)
    ...
    def setAvatar(self, imagePath):
        return imports.User(self.discord, self.s,
                            self.log).setAvatar(imagePath)

    # set profile color
    def setProfileColor(self, color=None):
        return imports.User(self.discord, self.s,
                            self.log).setProfileColor(color)

    # set username
    def setUsername(
            self,
            username):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s, self.log).setUsername(
            username, password=self.__user_password)

    # set email
    def setEmail(self,
                 email):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        return imports.User(self.discord, self.s,
                            self.log).setEmail(email,
                                               password=self.__user_password)

    # set password
    def setPassword(
            self,
            new_password):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
    ...
    def setDiscriminator(
            self,
            discriminator):  # USER PASSWORD NEEDS TO BE SET BEFORE THIS IS RUN
        ...
        if secret == "default":
            if self.__totp_secret == "":
                self.__totp_secret = "".join(
                    ...
                    for _ in range(16))  # random base32 (len 16)
            secret = self.__totp_secret
        if "?secret=" in secret:
            secret = secret[secret.index("?secret=") +
                            8:secret.index("?secret=") + 24]
        return (
    ...
    def disable2FA(
        self,
        code="calculate",
        clearSecretAfter=False
    ):  # either set your token before running this or input a code.
        if code == "calculate":
            code = self.calculateTOTPcode(
            )[0]  # this will generate a random secret if you dont have one set, so...set your secret before running this
    ...
    def getBackupCodes(self, regenerate=False):
        return imports.User(self.discord, self.s,
                            self.log).getBackupCodes(self.__user_password,
                                                     regenerate)

    def disableAccount(self, password):
        return imports.User(self.discord, self.s,
                            self.log).disableAccount(password)

    def deleteAccount(self, password):
        return imports.User(self.discord, self.s,
                            self.log).deleteAccount(password)

    def setPhone(self, number, reason="user_settings_update"):
        return imports.User(self.discord, self.s,
                            self.log).setPhone(number, reason)

    def validatePhone(self, number, code, password):
        result = imports.User(self.discord, self.s,
                              self.log).validatePhone(number, code, password)
    ...
    def allowDMsFromServerMembers(self, allow=True, disallowedGuildIDs=None):
        return imports.User(self.discord, self.s,
                            self.log).allowDMsFromServerMembers(
                                allow, disallowedGuildIDs)

    def allowFriendRequestsFrom(self,
                                types=[
                                    "everyone", "mutual_friends",
                                    "mutual_guilds"
                                ]):
        return imports.User(self.discord, self.s,
                            self.log).allowFriendRequestsFrom(types)

    def analyticsConsent(self, grant=[], revoke=[]):
        return imports.User(self.discord, self.s,
                            self.log).analyticsConsent(grant, revoke)

    def allowScreenReaderTracking(self, allow=True):
        return imports.User(self.discord, self.s,
                            self.log).allowScreenReaderTracking(allow)
    ...
    def getConnectedAccounts(self):
        return imports.User(self.discord, self.s,
                            self.log).getConnectedAccounts()

    def getConnectionUrl(self, accountType):
        return imports.User(self.discord, self.s,
                            self.log).getConnectionUrl(accountType)

    def enableConnectionDisplayOnProfile(self,
                                         accountType,
                                         accountUsername,
                                         enable=True):
        return imports.User(self.discord, self.s,
                            self.log).enableConnectionDisplayOnProfile(
                                accountType, accountUsername, enable)

    def enableConnectionDisplayOnStatus(self,
                                        accountType,
                                        accountUsername,
                                        enable=True):
        return imports.User(self.discord, self.s,
                            self.log).enableConnectionDisplayOnStatus(
                                accountType, accountUsername, enable)

    def removeConnection(self, accountType, accountUsername):
        return imports.User(self.discord, self.s,
                            self.log).removeConnection(accountType,
                                                       accountUsername)

    def getBillingHistory(self, limit=20):
        return imports.User(self.discord, self.s,
                            self.log).getBillingHistory(limit)
    ...
    def getBillingSubscriptions(self):
        return imports.User(self.discord, self.s,
                            self.log).getBillingSubscriptions()

    def getStripeClientSecret(self):
        return imports.User(self.discord, self.s,
                            self.log).getStripeClientSecret()
    ...
    def enableDevMode(self, enable=True):  # boolean
        return imports.User(self.discord, self.s,
                            self.log).enableDevMode(enable)

    def activateApplicationTestMode(self, applicationID):
        return imports.User(
            self.discord, self.s,
            self.log).activateApplicationTestMode(applicationID)
    ...
    def enableActivityDisplay(self, enable=True):
        return imports.User(self.discord, self.s,
                            self.log).enableActivityDisplay(enable)
    ...
    def reportSpam(
        ...
        return imports.User(self.discord, self.s,
                            self.log).reportSpam(channelID, messageID,
                                                 reportType, guildID, version,
                                                 variant, language)

    def getHandoffToken(self, key):
        return imports.User(self.discord, self.s,
                            self.log).getHandoffToken(key)

    def inviteToCall(self, channelID, userIDs=None):
        return imports.User(self.discord, self.s,
                            self.log).inviteToCall(channelID, userIDs)

    def declineCall(self, channelID):
        return imports.User(self.discord, self.s,
                            self.log).declineCall(channelID)
    ...
    def setMemberRoles(self, guildID, memberID, roleIDs):
        return imports.Guild(self.discord, self.s,
                             self.log).setMemberRoles(guildID, memberID,
                                                      roleIDs)

    def getMemberVerificationData(self,
                                  guildID,
                                  with_guild=False,
                                  invite_code=None):
        return imports.Guild(self.discord, self.s,
                             self.log).getMemberVerificationData(
                                 guildID, with_guild, invite_code)

    def agreeGuildRules(self,
                        guildID,
                        form_fields,
                        version="2021-01-05T01:44:32.163000+00:00"):
        return imports.Guild(self.discord, self.s,
                             self.log).agreeGuildRules(guildID, form_fields,
                                                       version)
    ...
    def createThread(self,
        ...
        return imports.Guild(self.discord, self.s,
                             self.log).createThread(channelID, name, messageID,
                                                    public, archiveAfter)

    def leaveThread(self, threadID, location="Sidebar Overflow"):
        return imports.Guild(self.discord, self.s,
                             self.log).leaveThread(threadID, location)

    def joinThread(self, threadID, location="Banner"):
        return imports.Guild(self.discord, self.s,
                             self.log).joinThread(threadID, location)

    def archiveThread(self, threadID, lock=True):
        return imports.Guild(self.discord, self.s,
                             self.log).archiveThread(threadID, lock)

    def unarchiveThread(self, threadID, lock=False):
        return imports.Guild(self.discord, self.s,
                             self.log).unarchiveThread(threadID, lock)

    def lookupSchool(self,
                     email,
                     allowMultipleGuilds=True,
                     useVerificationCode=True):
        return imports.Guild(self.discord, self.s,
                             self.log).lookupSchool(email, allowMultipleGuilds,
                                                    useVerificationCode)

    def schoolHubWaitlistSignup(self, email, school):
        return imports.Guild(self.discord, self.s,
                             self.log).schoolHubWaitlistSignup(email, school)

    def schoolHubSignup(self, email, hubID):
        return imports.Guild(self.discord, self.s,
                             self.log).schoolHubSignup(email, hubID)

    def verifySchoolHubSignup(self, hubID, email, code):
        return imports.Guild(self.discord, self.s,
                             self.log).verifySchoolHubSignup(
                                 hubID, email, code)

    def getSchoolHubGuilds(self, hubID):
        return imports.Guild(self.discord, self.s,
                             self.log).getSchoolHubGuilds(hubID)

    def getSchoolHubDirectoryCounts(self, hubID):
        return imports.Guild(self.discord, self.s,
                             self.log).getSchoolHubDirectoryCounts(hubID)

    def joinGuildFromSchoolHub(self, hubID, guildID):
        return imports.Guild(self.discord, self.s,
                             self.log).joinGuildFromSchoolHub(hubID, guildID)

    def searchSchoolHub(self, hubID, query):
        return imports.Guild(self.discord, self.s,
                             self.log).searchSchoolHub(hubID, query)

    def getMySchoolHubGuilds(self, hubID):
        return imports.Guild(self.discord, self.s,
                             self.log).getMySchoolHubGuilds(hubID)

    def setSchoolHubGuildDetails(self, hubID, guildID, description,
                                 directoryID):
        return imports.Guild(self.discord, self.s,
                             self.log).setSchoolHubGuildDetails(
                                 hubID, guildID, description, directoryID)

    def getLiveStages(self, extra=False):
        return imports.Guild(self.discord, self.s,
                             self.log).getLiveStages(extra)

    def getChannel(self, channelID):
        return imports.Guild(self.discord, self.s,
                             self.log).getChannel(channelID)
    ...
    def getSlashCommands(self, applicationID):
        return imports.SlashCommands(self.discord, self.s,
                                     self.log).getSlashCommands(applicationID)
    ...
    def triggerSlashCommand(
        ...
        return imports.SlashCommands(self.discord, self.s,
                                     self.log).triggerSlashCommand(
                                         applicationID, channelID, guildID,
                                         data, nonce, sessionID)
    ...
    def initScience(self):
        try:
            # get analytics token
            response = imports.User(self.discord, self.s,
                                    self.log).info(with_analytics_token=True)
