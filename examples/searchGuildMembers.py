...
def test(resp):
    ...
    if resp.event.guild_members_chunk and bot.gateway.finishedGuildSearch(
        ["guildID"], "a"):
        bot.gateway.close()
...
print(bot.gateway.guildMemberSearches["guildID"]["queries"]
      ["a"])  # user IDs of results
...
def test(resp):
    if resp.event.ready_supplemental:
        bot.gateway.checkGuildMembers(["guildID"], ["userID1", "userID2"],
                                      keep="all")
    if resp.event.guild_members_chunk and bot.gateway.finishedGuildSearch(
        ["guildID"], userIDs=["userID1", "userID2"]):
        bot.gateway.close()
...
bot.gateway.resetMembersOnSessionReconnect = (
    False  # member list brute forcing can take a while
)
...
class MemberFetchingScore:
    def __init__(self, perRequestExpectation, perSecondExpectation):
        self.perRequestExpectation = (
            perRequestExpectation  # expected # of members per request
        )
        self.perSecondExpectation = (
            perSecondExpectation  # expected # of members per second
        )
    ...
    def calculateEffectiveness(self, guildID):
        self.effectiveness = (
            100 * (len(bot.gateway.session.guild(guildID).members) /
                   len(bot.gateway.guildMemberSearches[guildID]["queries"])) /
            self.perRequestExpectation)
...
def calculateOption(guildID, action):  # action == 'append' or 'replace'
    if action == "append":
        lastUserIDs = bot.gateway.guildMemberSearches[guildID]["queries"][
            "".join(Queries.qList)]
        data = [
            bot.gateway.session.guild(guildID).members[i]
            for i in bot.gateway.session.guild(guildID).members
            if i in lastUserIDs
        ]
        lastName = sorted(
            set([
                re.sub(" +", " ", j["nick"].lower()) if
                (j.get("nick") and re.sub(" +", " ",
                                          j.get("nick").lower()).startswith(
                                              "".join(Queries.qList))) else
                re.sub(" +", " ", j["username"].lower()) for j in data
            ]))[-1]
        try:
        ...
        if Queries.qList[-1] in allchars:
            ...
            if " " in options and (
                    len(Queries.qList) == 1 or
                (len(Queries.qList) > 1 and Queries.qList[-2] == " ")
            ):  # cannot start with a space and cannot have duplicate spaces
                options.remove(" ")
            return options
        else:
            return None
...
def findReplaceableIndex(guildID):
    for i in range(len(Queries.qList) - 2, -1,
                   -1):  # assume that the last index is not changable
        if Queries.qList[i] != "~":
            return i
    return None
...
def bruteForceTest(resp, guildID, wait):
    if resp.event.ready_supplemental:
        ...
        bot.gateway.queryGuildMembers([guildID],
                                      query="".join(Queries.qList),
                                      limit=100,
                                      keep="all")
    elif resp.event.guild_members_chunk:
        remove = False
        if (len(bot.gateway.guildMemberSearches[guildID]["queries"]["".join(
                Queries.qList)]) == 100):  # append
        ...
        print("members fetched so far: " +
              repr(len(bot.gateway.session.guild(guildID).members)))
