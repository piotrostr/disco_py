...
def calculateOption(guildID, action):  # action == 'append' or 'replace'
    if action == "append":
        ...
        lastName = sorted(
            set(
                [
                    ...
                    if (
                        j.get("nick")
                        and re.sub(" +", " ", j.get("nick").lower()).startswith(
                            "".join(Queries.qList)
                        )
                    )
                    else re.sub(" +", " ", j["username"].lower())
                    for j in data
                ]
            )
        )[-1]
        ...
        if Queries.qList[-1] in allchars:
            ...
            if " " in options and (
                len(Queries.qList) == 1
                or (len(Queries.qList) > 1 and Queries.qList[-2] == " ")
...
def findReplaceableIndex(guildID):
    for i in range(
        len(Queries.qList) - 2, -1, -1
    ):  # assume that the last index is not changable
        if Queries.qList[i] != "~":
            return i
    return None
...
def bruteForceTest(resp, guildID, wait):
    if resp.event.ready_supplemental:
        ...
        bot.gateway.queryGuildMembers(
            [guildID], query="".join(Queries.qList), limit=100, keep="all"
        )
    elif resp.event.guild_members_chunk:
        remove = False
        if (
            len(
                bot.gateway.guildMemberSearches[guildID]["queries"][
                    "".join(Queries.qList)
                ]
            )
            == 100
                ...
                bot.gateway.removeCommand(
                    {
                        ...
                        "params": {"guildID": guildID, "wait": wait},
                    }
                )
...
wait = 1
bot.gateway.command(
    {"function": bruteForceTest, "params": {"guildID": guildID, "wait": wait}}
)
bot.gateway.run()
